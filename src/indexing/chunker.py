...
def chunk_pages(pages: list[dict], chunk_size: int = 400, overlap: int = 80) -> list[Chunk]:
    ...
    for page in pages:
        ...
        # Prepend context header to the text so every chunk carries document context
...
def load_and_chunk(
    ...
    # Global chunks file
    global_path = processed_dir / "chunks.json"
    with open(global_path, "w") as f:
        json.dump([asdict(c) for c in chunks], f, indent=2)
    print(f"Created {len(chunks)} chunks from {len(pages)} pages → {global_path}")

    # Per-category chunk files (for folder-specific searching)
