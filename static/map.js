...
// Cache footway via-nodes so each destination is only queried once per session
const _footwayCache = new Map();

async function findFootwayVia(lat, lng) {
  const key = `${lat.toFixed(4)},${lng.toFixed(4)}`;
  if (_footwayCache.has(key)) return _footwayCache.get(key);

  const pad = 0.0015; // ~150 m bounding box
  const query = `[out:json][timeout:5];way["highway"~"^(footway|path)$"](${lat - pad},${lng - pad},${lat + pad},${lng + pad});node(w);out;`;
  let result = null;
  try {
...
function decodePolyline6(str) {
  ...
  while (index < str.length) {
    ...
    coords.push([lng / 1e6, lat / 1e6]); // [lng, lat] for drawMultiRoute
  }
  return coords;
}

async function valhallaWalkRoute(from, to) {
  // Snap destination to the nearest OSM footway node so Valhalla resolves
  // the endpoint onto the campus footway network rather than a road edge.
  const snapped = await findFootwayVia(to[0], to[1]).catch(() => null);
  ...
  const res = await fetch("https://valhalla1.openstreetmap.de/route", {
    ...
    body: JSON.stringify({
      ...
      costing_options: {
        pedestrian: {
          shortest: true,        // minimize distance, not time — prefers direct footpaths over faster roads
          walkway_factor: 0.1,   // strongly prefer dedicated footways
          use_tracks: 1.0,
          service_penalty: 0,
          alley_factor: 1.0,
          driveway_factor: 1.0,
